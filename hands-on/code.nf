 ...
 * (create an index file (.fai) for the input genome FASTA file.)
 */
 ...
 * (Utilizes Picard's CreateSequenceDictionary tool to generate a sequence dictionary
 * (.dict) for the input genome FASTA file. This dictionary is used by GATK (Genome Analysis Toolkit).)
 */
 ...
 * (Generates a genome index for STAR alignment using the STAR aligner tool)
 */
 ...
 * (Filters and recodes variants from a VCF (Variant Call Format) file using vcftools,
 * and compresses the output into a gzipped VCF file (.vcf.gz).)
 */
 ...
 * (Aligns RNA-Seq reads to the reference genome using the STAR aligner tool.)
 */
 ...
 * (Splits reads with N characters in their CIGAR string using GATK)
 */
 ...
 * (Recalibrates base quality scores of aligned reads using GATK)
 */
 ...
 * (Calls variants from aligned reads using GATK's HaplotypeCaller, and performs variant filtration)
 */
 ...
 * (Post-processes variant calls to identify allele-specific expression (ASE) and RNA editing events)
 */
 ...
 * (Quantifies allele-specific expression (ASE) using GATK's ASEReadCounter tool)
 */
